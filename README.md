...
**Quit Chrome before regenerating the share cards.** `tools/gen_og_images.py` drives headless Chrome, which shares the desktop browser's profile — a throwaway `--user-data-dir` never finishes starting up on macOS. When the desktop instance holds the profile lock, the headless run sometimes abandons the navigation and screenshots the new-tab page instead, and nothing about that looks like a failure: Chrome exits 0 and writes a valid 1200x630 PNG, so a Google new-tab screenshot lands in `assets/og/` wearing a card's file name. The script now checks that Chrome actually fetched each template from its own server and retries, then fails loudly rather than writing the wrong image — but quitting Chrome first avoids the retries entirely.
